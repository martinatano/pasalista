...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (saved === "trial" || saved === "simple" || saved === "negocio" || saved === "expired") setDevScenario(saved);
  }, []);
  ...
  function openLegacyPrintWindow() {
    ...
    const itemRows = order.items.map((item) => `<tr><td>${item.quantity}×</td><td><b>${escapeHtml(item.productName)}</b><small>${escapeHtml(item.productCode)}</small></td><td>${pesos.format(item.unitPrice * item.quantity)}</td></tr>`).join("");
  ...
  async function startSubscription(plan: "simple" | "negocio") {
    if (devScenario) {
      applyDevScenario(plan);
      setNotice(`Simulación activada: plan ${plan === "negocio" ? "Negocio" : "Simple"}. No se realizó ningún cobro.`);
      return;
    }
    ...
    setSaving(true); setNotice("");
    try {
      const response = await authenticatedFetch("/api/billing/checkout", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ plan, cycle: annualPricing ? "annual" : "monthly", email }) });
      const data = await response.json() as { checkoutUrl?: string; upgraded?: boolean; plan?: "negocio"; billingCycle?: "monthly" | "annual"; error?: string };
      if (!response.ok) throw new Error(data.error || "No pudimos actualizar tu plan.");
      if (data.upgraded) {
        setBilling((current) => ({ ...current, plan: "negocio", billingCycle: data.billingCycle || current.billingCycle, subscriptionStatus: "authorized" }));
        setNotice("Ya tenés Negocio. El nuevo importe se cobrará en tu próxima renovación.");
        return;
      }
  ...
  async function printSelectedOrder() {
    ...
    try {
      ...
      for (const item of order.items) {
        ...
        page.drawText(pesos.format(item.unitPrice * item.quantity), { x: 475, y, size: 9, font: bold, color: ink });
        y -= 32; rule(); y -= 10;
      }
      if (y < 130) newPage();
      y -= 8; draw("TOTAL", margin, 10, bold, muted); page.drawText(pesos.format(order.total), { x: 420, y, size: 19, font: bold, color: ink }); y -= 34;
      ...
      {view === "business" ? (
        <section className="business-layout">
          <aside>
            ...
            <button data-icon="◎" className={section === "customers" ? "active" : ""} onClick={() => canUseNegocio ? setSection("customers") : openUpgrade()}>Clientes {!canUseNegocio && <span className="nav-lock" aria-label="Requiere plan Negocio">Negocio</span>}</button>
            <small className="nav-label">NEGOCIO</small>
            <button data-icon="◇" className={section === "settings" ? "active" : ""} onClick={() => setSection("settings")}>Configuración</button>
            <button data-icon="$" className={section === "billing" ? "active" : ""} onClick={() => { setSection("billing"); setNotice(""); }}>Plan y facturación</button>
            <div className="sidebar-help"><b>{trialActive ? `${trialDaysLeft} días de prueba` : paidPlanActive ? `Plan ${activeBilling.plan === "negocio" ? "Negocio" : "Simple"}` : "Prueba finalizada"}</b><span>{trialActive ? "Todas las funciones están habilitadas." : paidPlanActive ? "Tu suscripción está activa." : "Elegí un plan para continuar."}</span><button onClick={() => setSection("billing")}>{trialActive ? "Ver planes" : "Elegir plan"}</button></div>
          </aside>
          <div className="business-main">
            {isLocalPreview && <section className="dev-plan-switcher" aria-label="Simular estado de la suscripción"><div><span>Solo en esta computadora</span><b>Probar planes</b></div><div role="group" aria-label="Estado simulado"><button className={devScenario === "trial" ? "active" : ""} onClick={() => applyDevScenario("trial")}>Prueba</button><button className={devScenario === "simple" ? "active" : ""} onClick={() => applyDevScenario("simple")}>Simple</button><button className={devScenario === "negocio" ? "active" : ""} onClick={() => applyDevScenario("negocio")}>Negocio</button><button className={devScenario === "expired" ? "active" : ""} onClick={() => applyDevScenario("expired")}>Vencida</button>{devScenario && <button className="dev-plan-real" onClick={() => applyDevScenario(null)}>Volver a mi cuenta</button>}</div></section>}
            ...
            {needsBusinessSetup && section !== "billing" ? <form className="business-onboarding" onSubmit={saveBusinessSetup}>
              ...
              <div className="settings-layout">
                <form className={`settings-form ${accountPaused ? "settings-paused" : ""}`} aria-disabled={accountPaused} onSubmit={saveSettings}>
                  <section><h2>Identidad del negocio</h2><p>Usá el nombre y los colores que tus clientes ya reconocen.</p><div className="settings-fields"><label className="wide">Nombre del negocio<input required maxLength={80} value={settings.name} onChange={(event) => setSettings({ ...settings, name: event.target.value, slug: slugFrom(event.target.value) })} /></label><label>Color de marca<span className="color-field"><input type="color" value={settings.brandColor} onChange={(event) => setSettings({ ...settings, brandColor: event.target.value })} /><code>{settings.brandColor}</code></span></label><label>Logo<span className="logo-upload"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={uploadLogo} /><span>{settings.logoKey ? "Cambiar imagen" : "Subir imagen"}</span><small>PNG, JPG o WebP · máximo 2 MB</small></span></label></div></section>
                ...
                <aside className="catalog-preview" aria-label="Vista previa del catálogo"><span>Vista previa</span><div className="preview-phone" style={{ "--brand-preview": settings.brandColor } as React.CSSProperties}><div className="preview-store">{settings.logoKey ? <img src={`/api/logo?slug=${encodeURIComponent(settings.slug)}&v=${logoVersion}`} alt="Logo del negocio" /> : <b>{settings.name.charAt(0) || "R"}</b>}<div><strong>{settings.name || "Tu negocio"}</strong><small>Lista Comercios</small></div></div><div className="preview-search">Buscar productos…</div><div className="preview-item"><span></span><div><b>Producto de ejemplo</b><small>Presentación · código</small></div><strong>$12.500</strong></div><div className="preview-order"><span>Pedido mínimo</span><b>{pesos.format(settings.minimumOrder)}</b></div></div></aside>
              </div>
            </>}
          </div>
        </section>
      ) : (
        <section className="customer-view" style={{ "--customer-brand": settings.brandColor } as React.CSSProperties}><div className="store-header"><div className="store-identity">{settings.logoKey ? <img src={`/api/logo?slug=${encodeURIComponent(settings.slug)}&v=${logoVersion}`} alt={`Logo de ${businessName}`} /> : <span>{businessName.charAt(0) || "R"}</span>}<div><b>{businessName}</b><small>Lista Comercios · actualizada {lastUpdate.toLowerCase()}</small></div></div><small>Pedido mínimo {pesos.format(settings.minimumOrder)}</small></div><div className="customer-main">
          <div className={`welcome ${accountPaused ? "preview-paused" : ""}`}><div><b>{accountPaused ? "Vista previa del catálogo" : "Hola, Don José 👋"}</b><small>{accountPaused ? "Tus clientes ven un aviso de catálogo pausado." : "Entraste desde tu enlace privado · Lista Comercios"}</small></div><button className="primary" onClick={() => { if (accountPaused) { setView("business"); setSection("billing"); } else repeatOrder(); }}>{accountPaused ? "Reactivar catálogo" : "Repetir último pedido"}</button></div>
          {notice && <div className="notice" role="status">{notice}</div>}
          ...
          <div className="product-grid">{pagedProducts.map((product) => { const qty = quantities[product.code] ?? 0; const image = productImage(product); return <article className={`product ${accountPaused ? "preview-product" : ""}`} key={product.code}><span className="product-emoji">{image ? <img src={image} alt={product.name} /> : productEmoji(product.name, product.category, product.emoji)}</span><div><b>{product.name}</b><small>{product.detail}{product.detail ? " · " : ""}{product.code}</small><strong>{pesos.format(product.price)}</strong></div><div className="quantity"><button disabled={accountPaused} onClick={() => updateQuantity(product.code, -1)} aria-label={`Quitar ${product.name}`}>−</button><span>{qty}</span><button disabled={accountPaused} onClick={() => updateQuantity(product.code, 1)} aria-label={`Agregar ${product.name}`}>+</button></div></article>; })}</div>
