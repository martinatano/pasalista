...
export default function PublicCatalog({ slug }: { slug: string }) {
  ...
  return <main className="public-catalog" style={{ "--customer-brand": business.brandColor } as React.CSSProperties}>
    ...
    <header className="store-header"><div className="store-identity">{business.logoKey ? <img src={`/api/logo?slug=${encodeURIComponent(slug)}`} alt={`Logo de ${business.name}`} /> : <span>{business.name.charAt(0)}</span>}<div><b>{business.name}</b><small>Catálogo mayorista</small></div></div><small>{business.minimumOrder > 0 ? `Pedido mínimo ${money.format(business.minimumOrder)}` : "Sin pedido mínimo"}</small></header>
