...
type Business = { name: string; slug: string; whatsapp: string; brandColor: string; minimumOrder: number; deliveryZones: string; deliveryDays: string; logoKey?: string | null };
const pesos = new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS", maximumFractionDigits: 0 });
const normalize = (value: string) => value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
const productsPerPage = 24;
const demoBusiness: Business = { name: "Distribuidora El Buen Sabor", slug: "demo", whatsapp: "", brandColor: "#fa7c4a", minimumOrder: 80000, deliveryZones: "Zona Norte", deliveryDays: "Jueves" };
...
export default function PublicCatalog({ slug }: { slug: string }) {
  ...
  async function sendOrder() {
    ...
    try {
      ...
      const message = [`*Pedido — ${customerName.trim()}*`, customerPhone.trim() ? `Teléfono: ${customerPhone.trim()}` : "", deliveryAddress.trim() ? `Dirección: ${deliveryAddress.trim()}` : "", "", ...lines, "", `*Total estimado: ${pesos.format(total)}*`, `Entrega: ${delivery}`, notes.trim() ? `Observaciones: ${notes.trim()}` : ""].filter(Boolean).join("\n");
  ...
  return <main className="public-catalog" style={{ "--customer-brand": business.brandColor } as React.CSSProperties}>
    ...
    <header className="store-header"><div className="store-identity">{business.logoKey ? <img src={`/api/logo?slug=${encodeURIComponent(slug)}`} alt={`Logo de ${business.name}`} /> : <span>{business.name.charAt(0)}</span>}<div><b>{business.name}</b><small>Catálogo mayorista</small></div></div><small>{business.minimumOrder > 0 ? `Pedido mínimo ${pesos.format(business.minimumOrder)}` : "Sin pedido mínimo"}</small></header>
    <section className="customer-main public-main"><div className="public-welcome"><div><span>Catálogo actualizado</span><h1>Armá tu pedido</h1><p>Elegí los productos, revisá las cantidades y envialo listo por WhatsApp.</p></div></div>
      ...
      <div className="product-grid">{pagedProducts.map((product) => { const quantity = quantities[product.code] ?? 0; const image = product.imageKey ? `/api/product-image?slug=${encodeURIComponent(slug)}&code=${encodeURIComponent(product.code)}` : ""; return <article className={`product ${product.stock === 0 ? "sold-out" : ""}`} key={product.code}><span className="product-emoji">{image ? <img src={image} alt={product.name} /> : productEmoji(product.name, product.category, product.emoji)}</span><div><b>{product.name}</b><small>{product.detail}{product.detail ? " · " : ""}{product.code}</small><strong>{pesos.format(product.price)}</strong></div>{product.stock === 0 ? <em>Sin stock</em> : <div className="quantity"><button onClick={() => change(product.code, -1)} aria-label={`Quitar ${product.name}`}>−</button><span>{quantity}</span><button onClick={() => change(product.code, 1)} aria-label={`Agregar ${product.name}`}>+</button></div>}</article>; })}</div>
